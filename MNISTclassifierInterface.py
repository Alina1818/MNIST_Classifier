...
class CNNClassifierKerasStyle(TrainableNN):
    ...
    def forward(self, x):
        ...
        x = self.pool(F.relu(self.conv2(x)))  # Reduce spatial dimension 28 -> 14
        x = self.dropout1(x)
...
class MnistClassifier:
    ...
    def fit(self, train_data, train_labels=None, **kwargs):
        ...
        if isinstance(self.model, nn.Module):
            ...
            # Non-torch models (RandomForest) expect numpy arrays
            if train_labels is None:
...
cnn_model.fit(train_loader, val_data=test_loader, epochs=100, lr=0.001, device=device, early_stopping_patience=10)
...
# 3. Too high learning rate — visible loss oscillation
